...
use crate::{flow5tuple::Flow5Tuple, flow_stats::{self, FlowStats, FlowStatsResult}};
...
impl UEStats {
    ...
    pub fn new_packet(&mut self, pkt_ipv4: &[u8], orig_len: u16, ts: u64) {
        self.num_pkt += 1;
        ...
        if let Some(existing_flow) = self.flow_stats.get_mut(&flow) {
            ...
            if is_expired {
                let flow_result = existing_flow.get_result();
    ...
    pub fn flush_unfinished_flows(&mut self) {
        for (flow_key, flow_stats) in &mut self.flow_stats {
            let flow_result = flow_stats.get_result();
            self.finished_flow_stats.push(flow_result);
    ...
    pub fn end_and_encode_stats(&mut self) -> Option<Vec<u8>> {
        self.flush_unfinished_flows();
        ...
        for flow_result in &self.finished_flow_stats {
            let flow_result_bytes = flow_result.encode();
