...
pub struct ModelDeploymentRequest {
	pub model_id: ModelDeploymentID,
	pub action: ModelAction,
	pub blob: Option<ModelWeightsBlob>,
	pub scores: Option<ModelReputationScores>
}

impl PFCPModel for ModelDeploymentRequest {
	const ID: u16 = 200;

	fn encode(&self) -> Vec<u8> {
		let mut result = vec![];
		result.append(&mut self.model_id.encode());
		result.append(&mut self.action.encode());
		self.blob.as_ref().map(|o| result.append(&mut o.encode()));
		self.scores.as_ref().map(|o| result.append(&mut o.encode()));
		result
	}
	...
	fn decode(stream: &[u8]) -> Result<Self, PFCPError> where Self: Sized {
		...
		while stream.len() > 4 {
			...
			let curmsg = &stream[..length as usize];
			match msgtype {
				ModelDeploymentID::ID => { model_id = Some(ModelDeploymentID::decode(curmsg)?); }
				ModelAction::ID => { action = Some(ModelAction::decode(curmsg)?); }

				ModelWeightsBlob::ID => { blob = Some(ModelWeightsBlob::decode(curmsg)?); }
				ModelReputationScores::ID => { scores = Some(ModelReputationScores::decode(curmsg)?); }
				_ => { println!("Ignore unknown message type {}", msgtype); }
			}
			stream = &stream[length as usize..];
		}
		Ok(Self {
			model_id: model_id.ok_or(PFCPError::new("Missing mandatory field ModelDeploymentID"))?,
			action: action.ok_or(PFCPError::new("Missing mandatory field ModelAction"))?,
...
impl PFCPModel for ModelDeploymentResponse {
	const ID: u16 = 201;
