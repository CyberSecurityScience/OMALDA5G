...
impl PFCPHeader {
    ...
    pub fn is_request(&self) -> bool {
        match self.msg_type {
            1 | 3 | 5 | 7 | 9 | 12 | 14 | 50 | 52 | 54 | 56 | 200 => true,
            _ => false,
        }
    }
}
...
impl PFCPModel for UPFunctionFeatures {
        ...
        Self: Sized,
    {
        ...
        let mut stream = stream;
        if length != 6 {
            return Err(PFCPError::new(&format!("Expect length 6, got {}", length)));
        }
        let mut bytes = [0u8; 8];
...
impl PFCPModel for AgoUpfPerfReport {
    ...
    fn encode(&self) -> Vec<u8> {
        let mut result = Self::ID.to_be_bytes().to_vec();
        result.append(&mut 24u16.to_be_bytes().to_vec());
        ...
        Self: Sized,
    {
        ...
        let mut stream = stream;
        if length != 24 {
            return Err(PFCPError::new(&format!("Expect length 8, got {}", length)));
        }
...
bitfield! {
    ...
    /// REC5G (REport OMALDA5G): when set to "1", the SMF instructs the UPF to report the detection malware traffic.
    pub getREC5G, setREC5G: 9, 9;
}
...
#[derive(Debug, Clone, PartialEq)]
pub struct ModelDeploymentID {
    pub deployment_name: String,
    pub model_id: u8,
}

impl PFCPModel for ModelDeploymentID {
    const ID: u16 = 1100;

    fn encode(&self) -> Vec<u8> {
        let mut result = Self::ID.to_be_bytes().to_vec();
        result.append(&mut 2u16.to_be_bytes().to_vec());
        result.append(&mut (self.deployment_name.len() as u16).to_be_bytes().to_vec());
        result.append(&mut self.deployment_name.as_bytes().to_vec());
        result.push(self.model_id);
        let length: u16 = result.len() as u16 - 4;
        let length_be = length.to_be_bytes();
        result[2] = length_be[0];
        result[3] = length_be[1];
        ...
        Self: Sized,
    {
        ...
        let deployment_name_length = u16::from_be_bytes(stream[..2].try_into().unwrap()) as usize;
        stream = &stream[2..];
        let deployment_name = String::from_utf8(stream[..deployment_name_length].to_vec()).unwrap();
        stream = &stream[deployment_name_length..];
        let model_id = stream[0];
        stream = &stream[1..];
        Ok(Self {
            deployment_name,
            model_id,
...
impl PFCPModel for ModelAction {
    const ID: u16 = 1101;

    fn encode(&self) -> Vec<u8> {
        let mut result = Self::ID.to_be_bytes().to_vec();
        result.append(&mut 0u16.to_be_bytes().to_vec());
        result.push(self.action);
        self.dns_on_threshold
            .as_ref()
            .map(|o| result.append(&mut o.to_be_bytes().to_vec()));
        self.dns_off_threshold
            .as_ref()
            .map(|o| result.append(&mut o.to_be_bytes().to_vec()));
        self.dpi_threshold
            .as_ref()
            .map(|o| result.append(&mut o.to_be_bytes().to_vec()));
        self.dns_score_scale
            .as_ref()
            .map(|o| result.append(&mut o.to_be_bytes().to_vec()));
        let length: u16 = result.len() as u16 - 4;
        let length_be = length.to_be_bytes();
        ...
        Self: Sized,
    {
        ...
        let action = stream[0];
        stream = &stream[1..];
        let dns_on_threshold = if length >= 9 {
            Some(f64::from_be_bytes(stream[..8].try_into().unwrap()))
        } else {
            None
        };
        let dns_off_threshold = if length >= 17 {
            Some(f64::from_be_bytes(stream[8..16].try_into().unwrap()))
        } else {
            None
        };
        let dpi_threshold = if length >= 25 {
            Some(f64::from_be_bytes(stream[16..24].try_into().unwrap()))
        } else {
            None
        };
        let dns_score_scale = if length >= 33 {
            Some(f64::from_be_bytes(stream[24..32].try_into().unwrap()))
        } else {
            None
        };
...
impl PFCPModel for ModelWeightsBlob {
    const ID: u16 = 1102;
    ...
    fn encode(&self) -> Vec<u8> {
        ...
        result.append(&mut self.total_size.to_be_bytes().to_vec());
        result.append(&mut self.offset.to_be_bytes().to_vec());
        result.append(&mut self.chunk_id.to_be_bytes().to_vec());
        result.append(&mut self.total_chunks.to_be_bytes().to_vec());
        result.append(&mut self.chunk_length.to_be_bytes().to_vec());
        result.append(&mut self.chunk.clone());
        let length: u16 = result.len() as u16 - 4;
        let length_be = length.to_be_bytes();
        result[2] = length_be[0];
        result[3] = length_be[1];
        ...
        Self: Sized,
    {
        ...
        if length < 41 {
            return Err(PFCPError::new(&format!(
                "Expect length at least 41, got {}",
                length
            )));
        }
        let flags = stream[0];
        stream = &stream[1..];
        let total_size = u64::from_be_bytes(stream[..8].try_into().unwrap());
        stream = &stream[8..];
        let offset = u64::from_be_bytes(stream[..8].try_into().unwrap());
        stream = &stream[8..];
        let chunk_id = u64::from_be_bytes(stream[..8].try_into().unwrap());
        stream = &stream[8..];
        let total_chunks = u64::from_be_bytes(stream[..8].try_into().unwrap());
        stream = &stream[8..];
        let chunk_length = u64::from_be_bytes(stream[..8].try_into().unwrap());
        stream = &stream[8..];
        
        let chunk = stream[..chunk_length as usize].to_vec();
...
#[derive(Debug, Clone, PartialEq)]
pub struct ModelReputationScores {
    pub domain_scores: Vec<(String, i16)>,
    pub total_domains: u16,
}

impl PFCPModel for ModelReputationScores {
    const ID: u16 = 1103;

    fn encode(&self) -> Vec<u8> {
        let mut result = Self::ID.to_be_bytes().to_vec();
        result.append(&mut 0u16.to_be_bytes().to_vec());
        result.append(&mut self.total_domains.to_be_bytes().to_vec());
        result.append(&mut (self.domain_scores.len() as u16).to_be_bytes().to_vec());
        for (domain, score) in &self.domain_scores {
            result.append(&mut (domain.len() as u16).to_be_bytes().to_vec());
            result.append(&mut domain.as_bytes().to_vec());
            result.append(&mut score.to_be_bytes().to_vec());
        }
        let length: u16 = result.len() as u16 - 4;
        let length_be = length.to_be_bytes();
        result[2] = length_be[0];
        ...
        Self: Sized,
    {
        ...
        let total_domains = u16::from_be_bytes(stream[..2].try_into().unwrap());
        stream = &stream[2..];
        let mut domain_scores = vec![];
        let domain_count = u16::from_be_bytes(stream[..2].try_into().unwrap()) as usize;
        stream = &stream[2..];
        for _ in 0..domain_count {
            ...
            let domain_length = u16::from_be_bytes(stream[..2].try_into().unwrap()) as usize;
            stream = &stream[2..];
            if length < domain_length + 2 + 2 {
                return Err(PFCPError::new(&format!(
                    "Expect length at least {}, got {}",
                    domain_length + 2 + 2,
                    length
            ...
            let domain = String::from_utf8(stream[..domain_length].to_vec()).unwrap();
            stream = &stream[domain_length..];
            let score = i16::from_be_bytes(stream[..2].try_into().unwrap());
            stream = &stream[2..];
            domain_scores.push((domain, score));
        }
        Ok(Self {
            domain_scores,
            total_domains
...
bitfield! {
    ...
    /// REC5G (REport OMALDA5G): when set to "1", this indicates a malware detection report.
    pub getREC5G, setREC5G: 13, 13;
}
