use std::{collections::{HashMap, HashSet}, net::{IpAddr, Ipv4Addr, Ipv6Addr}, sync::{Arc, RwLock}};

use crate::{
	context::{self, UPF_PARAMETERS, get_async_runtime},
	datapath::{PacketPipeline, FlattenedFAR, FlattenedURR, tofino::TofinoBackendError, FlattenedQER},
};
...
impl SessionRequestHandlers for N4Handlers {
	fn handle_session_establishment(
		...
		let sess_handle = match super::allocate_pfcp_session(&request.cp_f_seid) {
			Some(s) => s,
			None => {
				response.cause = Cause::RequestRejectedUnspecified; // TODO: replace with correct error handling
		...
		let mut sess = sess_handle.write().unwrap();
		...
		let predefined_content_guard = super::PREDEFINED_RULES.read().unwrap();
		...
		let flatten_urrs = request.create_urr.iter().map(|urr| {
			FlattenedURR {
				...
				exempted_application_id_for_quota_action: urr.exempted_application_id_for_quota_action.clone(),
				exempted_sdf_filter_for_quota_action: urr.exempted_sdf_filter_for_quota_action.clone(),
				enable_dwd: urr.reporting_triggers.getREC5G() != 0
			}
		}).collect::<Vec<_>>();
	...
	fn handle_session_modification(
		...
		let mut pfcp_ctx_opt = super::GLOBAL_PFCP_CONTEXT.write().unwrap();
		let mut guard = context::BACKEND.write().unwrap();
		let be = guard.as_mut().unwrap();
		let pfcp_ctx = pfcp_ctx_opt.as_mut().unwrap();
		if let Some(sess_handle) = pfcp_ctx.PfcpSessions.get_mut(&local_seid) {
			let mut sess = sess_handle.write().unwrap();
			...
			let result_of_update = get_async_runtime().block_on(async {
				...
				be.update_or_add_forwarding(local_seid, updated_or_added_pipelines).await
			});
			...
			response.ago_perf = Some(libpfcp::models::AgoUpfPerfReport { stats1: stats.stats1, stats2: stats.stats2, stats3: perf_session_mod_elpased });
			response.encode()
		} else {
	...
	fn handle_session_deletion(
		...
		let (stats, reports) = super::delete_pfcp_session(local_seid);
