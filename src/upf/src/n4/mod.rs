...
impl PFCPSessionUP {
	fn send_impl(&self, msg: &Vec<u8>, matching_tuple: &ResponseMatchingTuple, dst_port_override: Option<u16>) -> PFCPRequestFuture {
		...
		let mut ongoings = libpfcp::PFCP_NODE_GLOBAL_CONTEXT.ongoing_requests.write().unwrap();
	...
	async fn send(&mut self, msg_type: u8, body: Vec<u8>, timeout_retry_count: Option<(std::time::Duration, u8)>, dst_port_override: Option<u16>) -> Result<(PFCPHeader, Vec<u8>), tokio::time::error::Elapsed> {
		...
		if let Some((timeout, retry_count)) = timeout_retry_count {
			...
			for _ in 0..retry_count {
				...
				if let Err(e) = ret {
					er.replace(e);
					libpfcp::PFCP_NODE_GLOBAL_CONTEXT.ongoing_requests.write().unwrap().remove(&matching_tuple);
					continue;
				} else {
...
pub fn allocate_pfcp_session(cp_f_seid: &F_SEID) -> Option<Arc<RwLock<PFCPSessionUP>>> {
	let upf_para = UPF_PARAMETERS.get().unwrap();
	let mut pfcp_ctx_opt = GLOBAL_PFCP_CONTEXT.write().unwrap();
	...
	let guard = PREDEFINED_RULES.read().unwrap();
...
pub fn find_pfcp_session_by_local_seid(id: u64) -> Option<Arc<RwLock<PFCPSessionUP>>> {
	let pfcp_ctx_opt = GLOBAL_PFCP_CONTEXT.read().unwrap();
...
pub fn delete_pfcp_session(seid: u64) -> (OperationPerfStats, Vec<BackendReport>) {
	let mut pfcp_ctx_opt = GLOBAL_PFCP_CONTEXT.write().unwrap();
	let pfcp_ctx = match pfcp_ctx_opt.as_mut() {
		Some(a) => a,
		None => {
			return (OperationPerfStats { stats1: 0, stats2: 0, stats3: 0 }, vec![]);
		}
	};
	if let Some(pfcp_session) = pfcp_ctx.PfcpSessions.remove(&seid) {
		let mut sess = pfcp_session.write().unwrap();
		...
		let (mut stats, reports) = get_async_runtime().block_on(async {
			context::BACKEND
				.write()
				.unwrap()
				.as_mut()
				.unwrap()
				.delete_session(sess.local_f_seid.seid).await
		});
		...
		(OperationPerfStats { stats1: 0, stats2: 0, stats3: 0 }, vec![])
	}
}
