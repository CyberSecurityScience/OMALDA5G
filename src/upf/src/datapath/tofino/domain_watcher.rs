...
async fn dwd_monitor(bfrt: Arc<RwLock<Option<bfruntime::BFRuntime>>>, ur_tx: Sender<BackendReport>) {
    ...
    let dns_on_threshold_i16 = ((dns_on_threshold / (1.0f64 - dns_on_threshold)).ln() * dns_score_scale).round() as i16;
    let dns_off_threshold_i16 = ((dns_off_threshold / (1.0f64 - dns_off_threshold)).ln() * dns_score_scale).round() as i16;
