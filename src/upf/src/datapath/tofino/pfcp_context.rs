...
pub struct PfcpContext {
    ...
    // transcation context
    pub pipelines_in_transcation: linear_map::LinearMap<pdr_id_t, TofinoPdrPipeline>,
...
impl PfcpContext {
    pub fn new(seid: u64) -> Self {
        Self {
            ...
            pipelines_in_transcation: linear_map::LinearMap::new(),
            enable_dwd: false
        }
    }
}
...
impl PfcpContext {
    pub fn prepare_update(&mut self) {
        self.pipelines_in_transcation = self.pipelines.clone();
    }
    pub fn merge_updates(&mut self, deleted_pipes: Vec<pdr_id_t>, updated_or_new_pipes: linear_map::LinearMap<pdr_id_t, PacketPipeline>) -> Result<(), TofinoBackendError> {
        for d in deleted_pipes.iter() {
            self.pipelines_in_transcation.remove(d);
        }
        for (pdr_id, pipeline) in updated_or_new_pipes.iter() {
            ...
            self.pipelines_in_transcation.insert(*pdr_id, tofino_pdr_pipe);
		}
        Ok(())
    }
    ...
    pub fn activate_pipelines(&mut self, touched: linear_map::LinearMap<pdr_id_t, u32>, untouched: Vec<pdr_id_t>) {
        ...
        self.pipelines = self.pipelines_in_transcation.clone();
...
impl PfcpContext {
    ...
    pub fn get_ue_ip(&self) -> Option<u32> {
        for (_, pipe) in self.pipelines.iter() {
            for match_instance in pipe.matches.iter() {
                if let MatchInstance::DownlinkFromN6Simple(t) = match_instance {
                    return Some(u32::from_be_bytes(t.key_ue_ip.octets()));
                }
            }
        }
        return None;
    ...
    pub fn get_ul_ma_id(&self) -> Option<u32> {
        for (_, pipe) in self.pipelines.iter() {
            for match_instance in pipe.matches.iter() {
                if let MatchInstance::UplinkToN6Simple(t) = match_instance {
                    return pipe.maid.clone();
                }
            }
        }
        return None;
