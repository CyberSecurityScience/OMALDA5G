...
use libpfcp::{messages::{AdditionalMonitoringTime, CreateFAR, CreatePDR, ForwardingParameters, ModelDeploymentRequest, UpdateFAR, UpdatePDR, UpdateQER, PDI}, models::{ApplyAction, DroppedDLTrafficThreshold, GateStatus, LinkedURR_ID, MeasurementInformation, MeasurementMethod, NumberOfReports, OuterHeaderRemoval, PFCPSMReqFlags, Precedence, SourceInterface, SuggestedBufferingPacketsCount, VolumeQuota, VolumeThreshold, FAR_ID, F_TEID, GBR, MBR, PDR_ID, QER_ID, QFI, RQI, *}};
...
pub trait BackendInterface {
	...
	async fn update_or_add_forwarding(&mut self, seid: u64, pipelines: Vec<PacketPipeline>) -> Result<OperationPerfStats, Box<dyn std::error::Error + Send + Sync>>;
	...
	async fn reset_all(&mut self, settings: BackendSettings);
