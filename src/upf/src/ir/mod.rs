...
async fn send_reports(reports: &Vec<BackendReport>) {
	...
	// println!("{:?}", reports);
	let global_pfcp_guard = crate::n4::GLOBAL_PFCP_CONTEXT.read().unwrap();
	...
	for (seid, reports) in sessions {
		if let Some(session_up_handle) = all_pfcp.PfcpSessions.get(&seid) {
			let mut session_up = session_up_handle.write().unwrap();
...
fn usage_reporting_receiver_thread(mut ur_receiver: Receiver<BackendReport>) {
	// accumulate reports spanning 1/10 second
	let mut cur_reports = vec![];
	let runtime = tokio::runtime::Builder::new_current_thread().thread_name("Tofino UPF usage reporting thread").enable_time().build().unwrap();
...
pub fn create_n4_usage_reporting_threads(ur_receiver: Receiver<BackendReport>) -> JoinHandle<()> {
	std::thread::spawn(|| {
		usage_reporting_receiver_thread(ur_receiver);
	})
}
