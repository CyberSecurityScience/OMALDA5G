...
pub fn populate_offpath_detection_egree_mirror(tofino_table_info: &P4TableInfo, port: u16) -> Vec<Update> {
	let mirror_session = 2u16;
	let dst_port = port as u32;
	let table_key = TableKey {
		fields: vec![
			KeyField {
				field_id: tofino_table_info.get_key_id_by_name("$mirror.cfg", "$sid"),
	...
	let table_data = TableData {
		...
		fields: vec![
			...
			DataField {
				field_id: tofino_table_info.get_data_id_by_name("$mirror.cfg", "$max_pkt_len"),
				value: Some(data_field::Value::Stream(400u16.to_be_bytes().to_vec()))
			},
		]
	};
	let table_entry = TableEntry {
