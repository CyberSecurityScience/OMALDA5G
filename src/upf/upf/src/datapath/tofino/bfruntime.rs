...
impl BFRuntime {
	pub async fn new(grpc_addr: String, client_id: u32, device_id: u32) -> Result<BFRuntime, Box<dyn std::error::Error + Sync + Send>> {
		println!("Connecting to BFRuntime server at {}", grpc_addr);
	...
	pub async fn sync_registers(&mut self, table_names: Vec<&str>) {
		let updates = table_names.iter().map(|table_name| {
			let table_op = TableOperation {
				table_id: self.table_info.get_table_by_name(table_name),
				table_operations_type: "Sync".into(),
			};
			let entity = Entity {
				entity: Some(entity::Entity::TableOperation(table_op))
			};
	...
	pub async fn read_register_i16(&mut self, table_name: &str, reg_name: &str, table_size: usize) -> Vec<i16> {
		let mut values = vec![0i16; table_size];
		let flag = TableFlags {
			from_hw: false,
			key_only: false,
			mod_del: false,
		...
		for resp_enitity in et {
			...
			match r {
				entity::Entity::TableEntry(t) => {
					...
					if idx >= table_size as u32 {
						break;
					}
					let d = &t.data.unwrap().fields[0];
					if d.field_id == ctr_id {
						let val = d.value.as_ref().unwrap();
						match val {
							data_field::Value::Stream(s) => {
								let val = match s.len() {
									2 => {
										let v = i16::from_be_bytes(s.as_slice().try_into().unwrap());
										v as i16
									}
									_ => unreachable!()
								};
