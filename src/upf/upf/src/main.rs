...
use crate::{datapath::{PacketPipeline, tofino::{TofinoBackend, qos::{QosAndPortData, QosPortData, QosQfiData}}, Report}, ir::create_n4_usage_reporting_threads, dataplane_interface::create_dataplane_interface};
...
fn populate_predefined_rules() {
	let mut guard = n4::PREDEFINED_RULES.write().unwrap();
...
async fn main() {
	...
	*crate::context::ASYNC_RUNTIME.lock().unwrap() = Some(Handle::current());
	...
	let mut threads = libpfcp::setup_global_pfcp_contexts(pfcp_handler, Some(8806));
	...
	let mut be = context::BACKEND.write().unwrap();
	let bfrt = be.as_mut().unwrap();
	bfrt.stop().await;
