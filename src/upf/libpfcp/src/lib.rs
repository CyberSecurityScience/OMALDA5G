...
use std::{collections::{HashMap, HashSet}, future::Future, net::UdpSocket, ops::{Add, AddAssign, BitAnd, BitAndAssign, Shl, ShlAssign}, task::{Poll, Waker}, thread::JoinHandle};
use std::sync::{Arc};
...
pub struct PFCPNodeManager {
	...
	pub managed_pfcp_sessions: HashMap<u64, Arc<RwLock<PFCPSessionCP>>>
}
...
impl PFCPSessionCP {
	...
	pub async fn release_with_report(&mut self) {
		let mut request = PFCPSessionDeletionRequest {};
		let (header, response_body) = match self.SendSessionDeletionRequest(request.encode()).await {
			Ok(a) => a,
			Err(e) => {
	...
	pub async fn transaction_commit(&mut self, rules: PFCPSessionRules) -> Result<(Vec<CreatedPDR>, Option<AgoUpfPerfReport>), ()> {
		...
		let (header, response_body) = match self.SendSessionModificationRequest(request.encode()).await {
			Ok(a) => a,
			Err(e) => {
...
pub struct PFCPNodeGlobalContext {
	pub ongoing_requests: std::sync::RwLock<HashMap<ResponseMatchingTuple, Arc<std::sync::RwLock<PFCPRequestFutureSharedState>>>>,
	pub receive_seq_counters: std::sync::RwLock<HashMap<std::net::IpAddr, u32>>
}

impl PFCPNodeGlobalContext {
	pub fn new() -> PFCPNodeGlobalContext {
		PFCPNodeGlobalContext {
			ongoing_requests: std::sync::RwLock::new(HashMap::new()),
			receive_seq_counters: std::sync::RwLock::new(HashMap::new()),
		}
	}
}
...
pub fn setup_global_pfcp_contexts<C: 'static>(handlers: C, request_recv_port_override: Option<u16>) -> Vec<JoinHandle<()>> where C: SessionRequestHandlers + NodeRequestHandlers + Send + Sync + Clone {
	let send_socket = UdpSocket::bind("0.0.0.0:0").unwrap();
	let send_socket_recv_thread = send_socket.try_clone().unwrap();
	PFCP_GLOBAL_REQUEST_SOCKET.set(send_socket).unwrap();
	handlers::create_handler_thread(handlers, send_socket_recv_thread, request_recv_port_override)
}
