use tokio::{sync::Mutex, task, time::Elapsed};

use async_trait::async_trait;
use crate::{messages::{AssociationSetupRequest, AssociationSetupResponse, CreatedPDR, HeartbeatRequest, HeartbeatResponse, ModelDeploymentRequest, ModelDeploymentResponse, PFCPSessionDeletionRequest, PFCPSessionEstablishmentRequest, PFCPSessionEstablishmentResponse}, models::{CPFunctionFeatures, Cause, ModelAction, ModelDeploymentID, ModelReputationScores, ModelWeightsBlob, NodeID, PDNType, RecoveryTimeStamp, UPFunctionFeatures, F_SEID}, IDAllocator, PFCPError, PFCPModel, PFCPSessionRules, PFCPSessionTransactionState};
...
impl PFCPNodeManager {
	fn send_impl(&self, msg: &Vec<u8>, matching_tuple: &ResponseMatchingTuple, dst_port_override: Option<u16>) -> PFCPRequestFuture {
		...
		let mut ongoings = super::PFCP_NODE_GLOBAL_CONTEXT.ongoing_requests.write().unwrap();
		
		if let Some(unfinished_ongoing) = ongoings.insert(matching_tuple.clone(), shared_state.clone()) {
			unreachable!();
		}
	...
	async fn send(&mut self, msg_type: u8, body: Vec<u8>, timeout_retry_count: Option<(std::time::Duration, u8)>, dst_port_override: Option<u16>) -> Result<(PFCPHeader, Vec<u8>), tokio::time::Elapsed> {
		...
		if let Some((timeout, retry_count)) = timeout_retry_count {
			...
			for _ in 0..retry_count {
				...
				if let Err(e) = ret {
					er.replace(e);
					super::PFCP_NODE_GLOBAL_CONTEXT.ongoing_requests.write().unwrap().remove(&matching_tuple);
					continue;
				} else {
	...
	pub async fn new(
		...
		let mut ret = PFCPNodeManager {
			...
			managed_pfcp_sessions: HashMap::new(),
	...
	pub async fn deploy_model(
		&mut self,
		model_id: ModelDeploymentID,
		action: ModelAction,
		blob: Option<ModelWeightsBlob>,
		scores: Option<ModelReputationScores>,
		dst_port_override: Option<u16>
	) -> Result<(), PFCPError> {
		let depolyment_request = ModelDeploymentRequest {
			model_id: model_id,
			action: action,
			blob: blob,
			scores: scores
		};
		match self.send(ModelDeploymentRequest::ID as u8, depolyment_request.encode(), Some((Duration::from_secs(5), 3)), dst_port_override).await {
			Ok((_, resp_body)) => {
				let mut resp_body = resp_body.clone();
				let resp = match ModelDeploymentResponse::decode(resp_body.as_mut_slice()) {
					Ok(a) => a,
					Err(_) => { return Err(PFCPError::new("Failed to decode ModelDeployment response")); }
				};
				Ok(())
			},
			Err(_) => Err(PFCPError::new("No ModelDeployment response"))
		}
	}
	pub async fn create_pfcp_session(&mut self, rules: Option<PFCPSessionRules>, pdn_type: Option<PDNType>) -> Result<(Arc<RwLock<PFCPSessionCP>>, Vec<CreatedPDR>), PFCPError> {
		...
		Ok((pfcp_sess_ptr, response.created_pdr))
	}
}
...
impl PFCPSessionCP {
	...
	pub async fn SendSessionEstablishmentRequest(&mut self, msg: Vec<u8>) -> Result<(PFCPHeader, Vec<u8>), tokio::time::Elapsed> {
		self.send(50, msg, Some((Duration::from_secs(5), 3)), self.dst_port_override).await
	}
	pub async fn SendSessionModificationRequest(&mut self, msg: Vec<u8>) -> Result<(PFCPHeader, Vec<u8>), tokio::time::Elapsed> {
		self.send(52, msg, Some((Duration::from_secs(5), 3)), self.dst_port_override).await
	}
	pub async fn SendSessionDeletionRequest(&mut self, msg: Vec<u8>) -> Result<(PFCPHeader, Vec<u8>), tokio::time::Elapsed> {
		self.send(54, msg, Some((Duration::from_secs(5), 3)), self.dst_port_override).await
	}
}
