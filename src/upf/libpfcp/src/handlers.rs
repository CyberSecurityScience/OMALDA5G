		...
		while content.len() != 0 && fo_flag_set {
			if let Ok((body, next_msg_pointer, header)) = PFCPHeader::decode(content) {
				...
				if header.is_request() {
					...
					// // TODO: put message handling to another thread
					// let mut response_body = match header.msg_type {
					// 	1 => handlers.handle_heartbeat(header, body, src_addr.ip()),
					// 	3 => handlers.handle_pfd_management(header, body, src_addr.ip()),
					// 	5 => handlers.handle_association_setup(header, body, src_addr.ip()),
					// 	7 => handlers.handle_association_update(header, body, src_addr.ip()),
					// 	9 => handlers.handle_association_release(header, body, src_addr.ip()),
					// 	12 => handlers.handle_node_report(header, body, src_addr.ip()),
					// 	14 => handlers.handle_session_set_deletion(header, body, src_addr.ip()),
					// 	50 => handlers.handle_session_establishment(header, body, src_addr.ip()),
					// 	52 => handlers.handle_session_modification(header, body, src_addr.ip()),
					// 	54 => handlers.handle_session_deletion(header, body, src_addr.ip()),
					// 	56 => handlers.handle_session_report(header, body, src_addr.ip()),
					// 	_ => unreachable!(),
					// };
					...
					// // send response back
					// recv_socket.send_to(resp_msg.as_slice(), src_addr).unwrap();
				} else {
